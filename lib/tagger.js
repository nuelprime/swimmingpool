...
import { FACTORIES, OTHER } from './factories.js';
...
const padOf = (factory) => {
  ...
  // unknown factories all bucket into 'other' so the launchpad filter stays usable;
  // /api/discover still reports the distinct addresses for classification.
  return f ? f.launchpad : (factory ? OTHER : null);
};
...
// Raw factory/creator addresses from cache → Map(ca → address). Unlike cachedTags this does not
// map through padOf, because callers sometimes need the address itself: when it isn't a known
// factory, it's the EOA that deployed the token — the dev wallet.
export async function rawFactories(cas) {
  const out = new Map();
  if (!R_URL || !R_TOK || !cas.length) return out;
  const list = cas.map(c => c.toLowerCase());
  for (let i = 0; i < list.length; i += 200) {
    const slice = list.slice(i, i + 200);
    const vals = await redis(['MGET', ...slice.map(c => `fac:${c}`)]);
    if (!Array.isArray(vals)) continue;
    vals.forEach((v, k) => { if (v) out.set(slice[k], String(v).toLowerCase()); });
  }
  return out;
}
