...
export const FACTORIES = {
  ...
  '0xf441cc979fa862f2674b9188a7b529cafd3ce204': {
    launchpad: 'dontblink', tokenParam: 'token', retired: true,   // scrapped — not worth index budget
  ...
  // Stonk Launcher (stonkbrokers.io) — CollectionTokenDeployer. WALL and MANCER both launched
  // here; $STONKBROKER is the pad's own token. mode:'tx' because this contract declares NO events
  // at all — one external `deploy` and nothing emitted — so the log walk finds nothing. Walking its
  // transactions recovers the tokens and the dev wallet together. Public launch imminent.
  // lemon.fun's SECOND factory. Their API only lists tokens from TokenDeployer (0x9Af53520…), so
  // their own $LEMON.FUN came through gecko wearing no pad at all. It was deployed by this, a
  // verified contract named LemonLaunchFactory. Same mode:'tx' — it emits nothing either.
  '0x2ba793fd69bf251fd1af90b576be8b9fa6be46db': { launchpad: 'lemon', mode: 'tx' },

  '0x662003bf6049e36b4e887d47b8df8718ffbbc6c2': {
    launchpad: 'stonk', mode: 'tx',
    // no per-token URL confirmed on their site yet — /wall was a one-off sale page, so no `site`
    // rather than a guessed pattern that 404s.
  },
...
// DEX → LAUNCHPAD hint. GeckoTerminal's dex ids on this chain are launchpad-branded, and the
// dex comes free with pool data we already fetch — no extra calls, no rate limit.
// Only used where the factory is unknown; the deploying factory always wins.
// pools.fun stated every one of its tokens deploys straight into a SushiSwap pool, which is why
// sushiswap maps here — it's the one pad that declared its venue.
export const DEX_PAD = {
  'uniswap-pools-trade':       'pools.trade',
  'pons-v2-dex':               'pons',
  'pons-v2':                   'pons',
  'pons-dot-family':           'pons',
  'sushiswap-v3-robinhood':    'pools.fun',
  'sushiswap':                 'pools.fun',
  'bankr-robinhood':           'bankr',
  'clanker-robinhood':         'clanker',
  // 'up-v3': 'up',  — scrapped as a hood: one row, no activity. Its pools fall to 'other',
  //                     so the token is still findable, it just stops taking a dropdown slot.
};
