// HOLDER COUNTS + ICONS — launchpad-agnostic, from Blockscout's token endpoint (`holders_count`).
...
const TTL = 36 * 3600;   // holders drift slowly. A 6h TTL meant cached counts expired faster
                         // than the cron could refresh 700 tokens, so they kept regressing to '—'.
...
// Map(caLower → url) of cached token icons
export async function cachedIcons(cas) {
  const out = new Map();
  if (!R_URL || !R_TOK || !cas.length) return out;
  const list = cas.map(c => c.toLowerCase());
  for (let i = 0; i < list.length; i += 200) {
    const slice = list.slice(i, i + 200);
    const vals = await redis(['MGET', ...slice.map(c => `icon:${c}`)]);
...
export async function resolveHolders(cas, limit = 25) {
  ...
  const writes = [];
  const chunk = 16;   // Blockscout tolerates this; matters when clearing a large backlog
  for (let i = 0; i < todo.length; i += chunk) {
    await Promise.all(todo.slice(i, i + chunk).map(async ca => {
      try {
        const r = await fetch(BS + ca, { headers: { 'user-agent': 'Mozilla/5.0' }, signal: AbortSignal.timeout(5000) });
        if (!r.ok) return;
        const j = await r.json();
        const h = j.holders_count ?? j.holders ?? null;
        if (h != null) writes.push(`hold:${ca.toLowerCase()}`, String(h));
        // same response carries the token icon — cache it, since pools.trade ships emoji
        // instead of logos and dexscreener only covers some tokens.
        const ic = j.icon_url;
        if (ic && /^https:/.test(ic)) writes.push(`icon:${ca.toLowerCase()}`, ic);
      } catch {}
    }));
  }
