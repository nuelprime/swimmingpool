...
// pools.trade serves token art as ipfs:// URIs (81 of 100 in a typical list response) — the
// previous /^https:/ test silently discarded all of them, which is why new pools.trade pairs
// looked image-less despite showing fine on their own site.
function logoUrl(v) {
  if (!v) return null;
  const u = String(v);
