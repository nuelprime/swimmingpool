  ...
  if (Array.isArray(all)) {
    for (let i = 1; i < all.length; i += 2) {
      try {
        ...
        if ((!launchpad || t.launchpad === launchpad) && (t.deployer || '').toLowerCase() === want) {
          out.push({ ca: t.ca, sym: t.sym || t.ca.slice(0,6)+'…', name: t.name || null, launchpad: t.launchpad, createdAt: t.ts, mcapUsd: null });
        }
      } catch {}
    }
  }
